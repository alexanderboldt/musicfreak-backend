...
object Fixtures {
    object Artist {
        object Domain {
            val korn = Artist(0, "Korn", "image.jpg",Timestamp.from(Instant.now()), Timestamp.from(Instant.now()))
            val slipknot = Artist(0, "Slipknot", "image.jpg", Timestamp.from(Instant.now()), Timestamp.from(Instant.now()))
