...
fun ApiModelAlbum.newDbModel() = DbModelAlbum(0, artistId!!, name!!, year!!, tracks!!, Timestamp.from(Instant.now()), Timestamp.from(Instant.now()))

fun ApiModelAlbum.mergeDbModel(existing: DbModelAlbum) = DbModelAlbum(existing.id, artistId!!, name!!, year!!, tracks!!, existing.createdAt, Timestamp.from(Instant.now()))
