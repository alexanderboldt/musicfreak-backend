...
fun ApiModelArtist.newDbModel() = DbModelArtist(0, name!!, Timestamp.from(Instant.now()), Timestamp.from(Instant.now()))

fun ApiModelArtist.mergeDbModel(existing: DbModelArtist) = DbModelArtist(existing.id, name!!, existing.createdAt, Timestamp.from(Instant.now()))
